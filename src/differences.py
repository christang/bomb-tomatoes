...
from models.users import Ages, Occupations
...
def recover_generation_gap(u_ids, m_ids, workspace):
    ...
    for m_id in m_ids:
        ratings = Ratings(workspace.ratings.for_subset(u_ids, [m_id]))
        stats = {
            group: Workspace.summary_stats(ratings.for_age_group(group, workspace.users))
            for group in workspace.users.age_groups
        }
        biases.append(stats)
    return biases
...
def recover_occupation_gap(u_ids, m_ids, workspace):
    ...
    for m_id in m_ids:
        ratings = Ratings(workspace.ratings.for_subset(u_ids, [m_id]))
        stats = {
            group: Workspace.summary_stats(ratings.for_occupation(group, workspace.users))
            for group in workspace.users.occupations
        }
        biases.append(stats)
    return biases
...
def main():
    ...
    selected = [(chi2.pvalues_[i], workspace.movies[inv_movies_map[i]]) for i in supports]
    selected.sort(key=lambda s: s[0])
    stats = recover_generation_gap(users_map.keys(), [m.ID for _, m in selected], workspace)
    for s1, s2 in zip(selected, stats):
        print s1[1], '\n', sorted([(Ages[l], s[1], round(s[2], 2)) for l, s in s2.items()],
                                  key=lambda i: -i[2])
    ...
    selected = [(chi2.pvalues_[i], workspace.movies[inv_movies_map[i]]) for i in supports]
    selected.sort(key=lambda s: s[0])
    stats = recover_occupation_gap(users_map.keys(), [m.ID for _, m in selected], workspace)
    for s1, s2 in zip(selected, stats):
        print s1[1], '\n', sorted([(Occupations[l], s[1], round(s[2], 2)) for l, s in s2.items()],
                                  key=lambda i: -i[2])
